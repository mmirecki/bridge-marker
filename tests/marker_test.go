...
import (
	...
	"github.com/mmirecki/bridge-marker/tests"
...
var _ = Describe("bridge-marker", func() {
	Describe("bridge resource reporting", func() {
		It("should be reported only when available on node", func() {
			...
			out, err := tests.RunOnNode("node01", "sudo ip link add br-test type bridge")
			if err != nil {
				panic(fmt.Errorf("%v: %s", err, out))
			}

			defer tests.RunOnNode("node01", "sudo ip link del br-test")

			Eventually(func() bool {
				node, err := clientset.CoreV1().Nodes().Get("node01", v1.GetOptions{})
				Expect(err).ToNot(HaveOccurred())

				capacity, reported := node.Status.Capacity["bridge.network.kubevirt.io/br-test"]
			...
			out, err = tests.RunOnNode("node01", "sudo ip link del br-test")
			if err != nil {
				panic(fmt.Errorf("%v: %s", err, out))
			}
